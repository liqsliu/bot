...
async def _(client, message):
    ...
    if cmd[0] == "rss":
        if len(cmd) == 1:
            ...
            #      await client.send_message(message.chat_id, str(feed_list))
            ...
            for i in news_list:
                username = None
                if db.existed("channel", i):
                    chat = db.get(i, table="channel")
                    if chat:
                        username = chat["username"]
                else:
                    peer = await get_peer(i, client=UB)
                    if peer:
                        if peer.username:
                            username = peer.username
                    else:
                        logger.error(f"can not get {i}")
                    print(f"need wait {i}")
                    await asyncio.sleep(3)
                if username:
                    info += f"\nhttps://rsshub.app/telegram/channel/{username}"
                else:
                    logger.error(f"ignore {i}")
            ...
            if len(cmd) == 2:
                await client.send_message(chat_id, str(news_list))
            elif cmd[2] == "del":
                if int(cmd[3]) in news_list:
                    news_list.remove(int(cmd[3]))
                    await client.send_message(chat_id, str(news_list))
                else:
                    await client.send_message(chat_id, "no")
            elif cmd[2] == "add":
                if not int(cmd[3]) in news_list:
                    ...
                    await client.send_message(chat_id, "exist")
            elif cmd[2] == "load":
                import ast
                news_list.clear()
                news_list.extend(
                    ast.literal_eval(message.text.split(' ', 3)[3]))
                await save_config()
                await client.send_message(chat_id, str(news_list))
        elif cmd[1] == "cid":
            global cid_btrss
            ...
            if len(cmd) > 2:
                if cmd[2] in feed_list:
                    ...
                    await client.send_message(chat_id, "need a rss url")
        elif cmd[1] == "add":
            if len(cmd) == 2:
                await client.send_message(chat_id, "rss add url [cid]")
                ...
                if not cmd[2] in feed_list:
                    ...
                    if d.entries:
                        ...
                        await client.send_message(chat_id, str(max_date))
                        ...
                        feed_list.pop(cmd[2])
                        await client.send_message(chat_id, "no entry,deleted")
                if len(cmd) == 4:
            ...
            if len(cmd) == 2:
                await client.send_message(chat_id, "rss up|update url")
            else:
                if not cmd[2] in feed_list:
                    await client.send_message(chat_id, "no url")
                else:
                    url = cmd[2]
                    request_headers = {'Cache-control': 'max-age=600'}
                    d = feedparser.parse(url, request_headers=request_headers)
            ...
            import ast
            feed_list = ast.literal_eval(text.split(' ', 2)[2])
            await client.send_message(chat_id, str(feed_list))
        elif cmd[1] == "del":
            res = None
            if cmd[2] in feed_list:
                if len(cmd) == 3:
            ...
            await client.send_message(chat_id, "deleted: {}".format(res))
            ...
            if not found:
                #          if not info:
                await client.send_message(chat_id, "None")
            else:
                await client.send_message(chat_id, info)
        else:
            await client.send_message(chat_id, "wtf cmd")

...
from .tools import my_exceptions_handler as exceptions_handler
...
#    info = entry.link + "\n<b>" + entry.title + "</b>"
    ...
    if len(feed_list[url]) > 3 and cid_ipfsrss in feed_list[url][3:]:
        info = "<b>" + entry.title + "</b>"
        info += "\n"
        info += entry.link
    else:
        info = "<b><a href=\""+entry.link + "\">" + entry.title + "</a></b>"
    ...
    info += "\n"
    info += entry.get( "updated", entry.get("published", "no time"))
    info += " "
    if "author" in entry:
        info += entry["author"] 
    elif "authors" in entry and entry[0] and "name" in entry[0]:
        info += entry["authors"][0]['name'] + ' ...'


    info2 = entry.get("description", entry.get("content", entry.get("summary", entry.get("value", None))))
    if info2:
        info1=info
        info = info + "\n" + info2
    if len(info) > 4096:
...
#    await bot.send_message(MY_ID, info,parse_mode="htm")
    if len(feed_list[url]) == 3:
        ...
        #  msg = [2, cid_btrss, info, {"parse_mode": "html"}]
        msg = [2, cid_btrss, info, {"parse_mode": enums.ParseMode.HTML}]
        ...
        for cid in feed_list[url][3:]:
            if info2:
                ...
                msg = [2, cid, info, {"parse_mode": enums.ParseMode.HTML}]
            await MSG_QUEUE.put(msg)
...
def get_feed_t1():
    ...
    for url in feed_list.copy():
        ...
        try:
            ...
            from .tools import my_traceback
            my_traceback(e)
            continue
        if d:
            if hasattr(d, 'status'):
...
@exceptions_handler
async def feed_loop():
    """get feed and send to tg"""
    global feed_list
    logger.warning("rss bot start")
...
#    last = time.time()
    ...
    async def gf(url):
        ...
        #  if url == "https://bangumi.tv/feed/blog/anime":
        #      put("rss: "+url+str(headers))
        #  if not headers:
        if headers is None:
            return
        async with sem:
            res = await http(url, headers=headers, timeout=wait-5, return_headers=True)
        ...
        async with sem2:
            if res is not None:
                ...
                logger.debug("got rss: " + url)
                if xml is not None:
                    if isinstance(xml, bytes):
                        xml = xml.decode(errors='ignore')
                    #  if url.startswith("https://www.ptt.cc/"):
                    #      if not xml.startswith('<?xml version="1.0" encoding="UTF-8"?>'):
                    #          put(f"bad xml {repr(xml[:32])}\nfrom: {url}")
                    return await asyncio.to_thread(parse_xml, xml), url
    ...
    while True:
        ...
        for coro in asyncio.as_completed(map(gf, feed_list.keys()),
            ...
            try:
                ...
                #                  url = i.href
                #  if url is None:
                #      logger.warning(f"skip send rss: {d.feed}\nreason: can't find url")
                #      put(f"skip send rss: {d.feed}\nreason: can't find url")
                #      continue
                #
                #  if url not in feed_list:
                #      for i in feed_list:
                #          if i.startswith(url) or i.split("://")[1].startswith(url.split('://')[1]):
                #              url1 = url
                #              url = i
                #      if url not in feed_list:
                #          logger.warning(f"bad rss url: {url1} -> {url}\nparsed: {d.feed}")
                #          put(f"bad rss url: {url1} -> {url}\nparsed: {d.feed}")
                #          continue
                #      else:
                #          logger.warning(f"need fix, bad rss url: {url1} -> {url}\nparsed: {d.feed}")

                logger.debug("start to parse xml from: {}".format(url))
                status = feed_list[url]
                if 0:
                    if d.get('etag', 0) or d.get('modified', 0):
                        status[0] = d.get('etag', 0)

...
from ..utils.tools import encode_base64, decode_base64, SH_PATH
...
from ..utils.telegram import text2link, get_id, text2tg, get_text_from_msg
from ..utils.telegram import DOWNLOAD_PATH
...
async def qr(event):
    ...
    if len(text.encode()) < 64:
        ...
        data = MARK+"qr_de"
        text_bt = "decode"
    buttons.append([Button.inline(text_bt, data=data)])
...
#    file = await event.client.upload_file(file)
    ...
    if not event.offset:
        pass
#            res.append(await builder.document(file, type="file", title="qr, send file", description="png", text=text, buttons=buttons))
#        res.append(await builder.document(file, type="file", title="qr, send file", description="png", buttons=buttons))


    #    res.append(await builder.article("insert space", text=info, description=info, buttons=buttons))
    return res
...
#    file = await event.client.upload_file(file)
    if len(text.encode()) < 64:
        ...
        text_bt = "see"
        buttons.append([Button.inline(text_bt, data=data)])
    buttons.append([Button.switch_inline('try same', query=event.text, same_peer=True)])
    buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])
    res.append(await builder.photo(file, include_media=True, buttons=buttons))
    return res
...
#    text = text[::-1]
    ...
    buttons = []
    buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])
    res.append(await builder.article("reverse and reverse", text=info, description=info, buttons=buttons))
    return res
    ...
    try:
        ...
        if info:
            ...
            if info:
                ...
                res.append(await builder.article('base64 decode ok', description=info, text=info))
                decode_ok = True
        else:
            info = "fail to decode"
...
#                data = link.split("//")[1].split(".")[0]
        ...
        buttons.append([Button.inline(text_bt, data=data)])
        if link:
            buttons.append([Button.url('ipfs', url=link)])
        buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])
...
async def morse(event):
    ...
    if info:
        info = split_text(info)
        res.append(await builder.article('morse decode ok', description=info, text=info))
        decode_ok = True
    else:
        info = "fail to decode"
...
async def ia(event):
    ...
    if url_only_re.match(text):
        ...
        res.append(await builder.article('error url', description=text, text="error url"))
        return res
...
#    link = await asyncio.to_thread(archivenow.push, text, "ia")
    ...
    buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])
    if link:
        res.append(await builder.article('send link', description="using archive.org", text=link, buttons=buttons))
    else:
...
#    link = await asyncio.to_thread(pastebin, text)
    ...
    buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])
    if link:
        res.append(await builder.article('send link', description=link, text=link, buttons=buttons))
    else:
        res.append(await builder.article('error', description=ERROR_INFO, text=info, buttons=buttons))
    return res
...
#            buttons.append([Button.url('ipfs', url=link)])
    buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])
    if link:
        res.append(await builder.article('send ipfs link', description=link, text=link, buttons=buttons))
    else:
        res.append(await builder.article('error', description=ERROR_INFO, text=info, buttons=buttons))
    return res
...
#    text = get_text(event, "h")
    ...
    try:
        ...
        if info is not None:
            info = split_text(info)
            res.append(await builder.article("decode ok", description=info, text=info))
            decode_ok = True
        else:
            info = "fail to decode"
#    except ValueError:
...
#                        data = link.split("//")[1].split(".")[0]
            ...
            if s:
                ...
                buttons.append([Button.url(text_bt, url=s.group("url"))])

#            if link:
            if len(info) > 4096:
                ipfs_bt = [Button.url('ipfs url', url=link)]
                buttons.append(ipfs_bt)
            buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])

#            if not link and len(info.encode()) > 4096:
            if not link and len(text.encode()) >= 64:
                res.append(await builder.article('error', text=info, description="msg is too long and ipfs/tg is not ok, this may be a bug", buttons=buttons))
            else:
                if len(info.encode()) <= 4096:
                    res.append(await builder.article('send hidden str', text=info, description="use sapce", buttons=buttons))
                if link:
                    des = "using link"
                else:
                    des = "msd has been saved"
                mask = "█████"
                res.append(await builder.article(mask, text=mask, description=des, buttons=buttons))
#                if link:
                if len(info) > 4096:
                    buttons.remove(ipfs_bt)
                    res.append(await builder.article('send link', description="msg is too long", text=link, buttons=buttons))
    return res
...
async def _(event):
    ...
    data = data.decode()
    info = None
    if data.startswith(MARK+"baf"):
        msg = await event.get_message()
        text = msg.raw_text
        if text and text[0] in chr_list:
            info = destr(text)
        else:
#        await event.answer("msg is on ipfs, reading...", cache_time=60)
            ipfs_cid = data[1:]
#            link = "https://{}.ipfs.infura-ipfs.io/?filename=1.txt".format(ipfs_cid)
            link = "https://{}.ipfs.infura-ipfs.io/".format(ipfs_cid)
            info = await http(link)
    elif data == MARK+"url":
        # for pastebin and ipfs
        msg = await event.get_message()
        link = msg.raw_text
#        url_data = await asyncio.to_thread(get_from_url, link)
        info = await http(link)
    elif data == MARK+"sp_sp":
        # sp sapce
        msg = await event.get_message()
        text = msg.raw_text
        info = destr(text)
    elif data == MARK+"bs_de":
        # base64 decode
        info = decode_base64((await event.get_message()).raw_text).decode()
    elif data == MARK+"ms_de":
        info = decode_morse((await event.get_message()).raw_text)
    elif data == MARK+"qr_de":
        info = await qr_decode(event)
    elif data[0] in chr_list:
        info = destr(data)
    elif data.startswith(MARK+"tg "):
        cmds = data.split(" ")
        if len(cmds) == 3:
            uid = int(cmds[1])
        ...
        if len(cmds) == 2 or (len(cmds) == 3 and event.sender_id == uid):
            link = cmds[-1]
            if link.isnumeric():
                link = int(link)
            info = await get_text_from_msg(link, event)
    else:
        msg = await event.get_message()
        text = msg.raw_text
        if text and text[0] in chr_list:
    ...
    try:
        ...
        print(f"len: {len(info.encode())=}")
        from ..utils.telegram import put
        put(f"len: {len(info)=}")
        put(f"len: {len(info.encode())=}")
        raise
...
async def get_help(builder, offset=None):
    res = []
    if offset:
        key = offset
        ...
        buttons = []
        buttons.append([Button.switch_inline('try', query=key+" ", same_peer=True)])
        res.append(await builder.article(helps[1], description=helps[0], text=cmds[key][1], buttons=buttons))
    else:
        for i in cmds:
            res += await get_help(builder, i)
    return res
...
@tg_exceptions_handler
async def _(event):
    mp("inline query cmd {}: {}: {}".format(__name__.split(".")[-1], event.sender_id, event.text))
    text = event.text
    res = []
    next_offset = None
    ...
    if key and key in cmds:
        if text is not None:
            ...
            if not event.text.startswith(f"{key} "):
                next_offset = get_offset(key)
        else:
            res = await get_help(builder, key)
        # if event.offset:
    else:
        pass
    ...
    if not res:
        if text:
            ...
            if not res:
                res.append(await builder.article('error', description=ERROR_INFO, text=ERROR_INFO))
        else:
            buttons = []
            for i in cmds:
                buttons.append([Button.switch_inline('try{}'.format(cmds[i][1].split(":", 1)[1]), query="{} ".format(i), same_peer=True)])
            buttons.append([Button.switch_inline('try all', same_peer=True)])
            res.append(await builder.article('help: click to send full info', description=cmds_help, text=cmds_help_text, buttons=buttons))
            res += await get_help(builder)
            res += [
                    await builder.article('my_id', description="send my id to the chat", text="my id: "+str(event.sender_id))
                    ]
            res.append(await builder.article('debug', description="dump msg: {}".format(event.offset), text=event.stringify()))
            res.append(await builder.article('src', description="source link", text=link))
...
#        print(dir(i))
        if hasattr(i.send_message, "message"):
            if i.send_message.message == ERROR_INFO:

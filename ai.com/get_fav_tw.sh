...
[[ -d "$SH_PATH/.git" ]] && {
  ...
  SH_PATH=${HOME}/.cache
  bash init.sh $SH_PATH
}


LP=${LP:-/var/www/dav/tmp}
DOMAIN=${DOMAIN:-wtfipfs.eu.org}
...
#          you-get -O "${filename%.*}" "$URL"
          ...
          [[ -n "$filename" ]] && youtube-dl --socket-timeout 55 -q --id "$URL"

          [[ -e "$filename" ]] || {
            [[ -z "$filename" ]] && filename="$(date "+%Y%m%d_%H%M%S").mp4"
            # you-get -O "${filename%.*}" "$URL" &>/dev/null
            you-get -t 55 -O "${filename%.*}" "$URL" &>/dev/null
          }
          [[ -e "$filename" ]] && [[ "$(du -b "$filename" | cut -f1)" -eq 0 ]] && rm "$filename"

          [[ -e "$filename" ]] || {
            echo "E:fail to download: $URL >to> $LP"
            echo "maybe need to restart rclone"
            return 1
          }
        }
...
msg_text="$msg_text$files2"
}

#nohup bash "$SH_PATH/bsm.sh" "Telegram" "$msg_text" 4243 &>> ~/tera/t.log &
#nohup bash "$SH_PATH/bsm.sh" "C Telegram" "$msg_text" 4240 &>/dev/null &
#nohup bash "$SH_PATH/bsm.sh" "C twitter" "$msg_text" 4240 &>/dev/null &
nohup bash "$SH_PATH/bsm.sh" "C twitter" "$msg_text" 4240 gateway1 &>/dev/null &
...
local tw_file="$tw_fav/${id}_${username}"
